...
module CS
  ...
  def self.install(country)
    ...
    File.foreach(MAXMIND_DB_FN) do |line|
      ...
      cities[rec[STATE]] = [] if !states.has_key?(rec[STATE])
    ...
    # sort
    cities = cities.sort.to_h
    states = states.sort.to_h
    ...
    File.write(states_fn, states.to_yaml)
    File.write(cities_fn, cities.to_yaml)
  ...
  def self.cities(state, country = nil)
    ...
    if blank?(@cities[country])
      ...
      install(country) if !File.exist? cities_fn
  ...
  def self.states(country)
    ...
    if blank?(@states[country])
      ...
      install(country) if !File.exist? states_fn
  ...
  def self.countries
    if !File.exist? COUNTRIES_FN
      # countries.yml doesn't exist, extract from MAXMIND_DB
      update_maxmind unless File.exist? MAXMIND_DB_FN
